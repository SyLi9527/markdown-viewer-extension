...
export function createInlineConverter({ 
  ...
  async function convertInlineNode(node: InlineNode, parentStyle: ParentStyle = {}): Promise<InlineResult | InlineResult[] | null> {
    ...
    const nodeStyle = node.style ? { ...parentStyle, ...node.style } : parentStyle;
    switch (node.type) {
      case 'text':
        return convertTextWithEmoji(node.value, nodeStyle);

      case 'strong':
        return await convertInlineNodes(node.children, { ...nodeStyle, bold: node.style?.bold ?? true });

      case 'emphasis':
        return await convertInlineNodes(node.children, { ...nodeStyle, italics: node.style?.italics ?? true });

      case 'delete':
        return await convertInlineNodes(node.children, { ...nodeStyle, strike: node.style?.strike ?? true });

      case 'superscript':
        return await convertInlineNodes(node.children, { ...nodeStyle, superScript: true });

      case 'subscript':
        return await convertInlineNodes(node.children, { ...nodeStyle, subScript: true });
      ...
      case 'inlineMath':
        return await convertInlineMath(node, nodeStyle);
      ...
      case 'html': {
        ...
        return convertTextWithEmoji(textContent, nodeStyle);
      }
