...
class DocxExporter {
  ...
  private async convertAstToDocx(ast: DOCXASTNode): Promise<FileChild[]> {
    ...
    for (const node of ast.children) {
      ...
      if (converted) {
        const convertedItems = Array.isArray(converted) ? converted : [converted];
        const hasTable = convertedItems.some((item) => item instanceof Table);
        lastNodeType = hasTable ? 'table' : node.type;
      } else {
    ...
    if (node.type === 'html' && this.tableConverter) {
      ...
      if (tableNodes && tableNodes.length > 0) {
        ...
        for (let i = 0; i < tableNodes.length; i++) {
          const table = await this.tableConverter.convertTable(tableNodes[i], listLevel);
          elements.push(table);
